export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/analyze') {
      ...
      const result = await env.AI.run('@cf/moondream/moondream3.1-9B-A2B', { task: 'query', image, question: String(body.question || 'Décris précisément cette image en français, lis le texte visible et relève les informations importantes.') })
      return Response.json({ description: result.description || result.response || result.answer || JSON.stringify(result) })
    }
